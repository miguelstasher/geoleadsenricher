-- =====================================================
-- GeoLeads Authentication & User Ownership Schema (FIXED)
-- =====================================================

-- 1. First, let's check the actual structure of your tables
...
-- 3. Add user ownership to leads table (using correct types)
ALTER TABLE leads 
...
ADD COLUMN IF NOT EXISTS duplicate_of BIGINT REFERENCES leads(id) ON DELETE SET NULL,
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'cold_calling_leads') THEN
        ...
        ADD COLUMN IF NOT EXISTS shared_with UUID[] DEFAULT '{}',
        ADD COLUMN IF NOT EXISTS is_duplicate BOOLEAN DEFAULT false,
        ADD COLUMN IF NOT EXISTS duplicate_of TEXT REFERENCES cold_calling_leads(id) ON DELETE SET NULL,
        ADD COLUMN IF NOT EXISTS duplicate_count INTEGER DEFAULT 0;
...
-- 8. Create function to detect duplicates (using correct ID types)
...
SELECT 
    ...
    COUNT(CASE WHEN COALESCE(l.is_duplicate, false) = false THEN 1 END) as unique_leads,
    COUNT(CASE WHEN COALESCE(l.is_duplicate, false) = true THEN 1 END) as duplicate_leads,
...
COMMENT ON COLUMN leads.duplicate_of IS 'ID of the original lead if this is a duplicate (BIGINT)';
...
CREATE OR REPLACE FUNCTION share_lead_with_user(lead_id BIGINT, user_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE leads 
...
CREATE OR REPLACE FUNCTION transfer_lead_ownership(lead_id BIGINT, new_owner_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
    UPDATE leads 
