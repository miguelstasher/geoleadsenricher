...
export async function POST(request: NextRequest) {
  try {
    console.log('🔧 Manual fix: Updating stuck searches...');
    
    // Get all searches that are stuck in 'in_process' for more than 2 minutes
    ...
    return NextResponse.json({ 
      success: true, 
      message: `Fixed ${fixedCount} stuck searches`,
      fixed: fixedCount,
